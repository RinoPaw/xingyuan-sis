"""Native authentication screens for the full-screen TUI."""
from __future__ import annotations

from dataclasses import dataclass
from pathlib import Path
import sys
import time
from typing import Mapping

from ..auth import (
    ADMIN_USERNAME,
    Identity,
    authenticate,
    change_password as set_password,
    clear_session,
    has_admin,
    initialize_admin,
    write_session,
)
from . import animation, keys, screen, theme
from .board import Board
from .text_edit import TextBuffer, input_mode, read_event


_FIELD_WIDTH = 40
_LABEL_WIDTH = 10


@dataclass(frozen=True)
class FieldSpec:
    key: str
    label: str
    secret: bool = False
    editable: bool = True


def login(db_path: Path | str | None) -> Identity | None:
    """Initialize the administrator when needed, then show the TUI login page."""
    if not has_admin() and not _initialize_admin(db_path):
        return None

    values = {"username": "", "password": ""}
    message = ""
    while True:
        submitted = _run_form(
            "login",
            values,
            database=_database_name(db_path),
            message=message,
        )
        if submitted is None:
            return None

        username = submitted["username"].strip()
        identity = authenticate(db_path, username, submitted["password"])
        if identity is None:
            values = {"username": username, "password": ""}
            message = "账号或密码错误。"
            continue

        if identity.must_change_password:
            updated = change_password(db_path, identity, forced=True)
            if updated is None:
                values = {"username": username, "password": ""}
                message = "请先修改密码。"
                continue
            identity = updated

        write_session(identity, db_path)
        return identity


def change_password(
    db_path: Path | str | None,
    identity: Identity,
    *,
    forced: bool = False,
) -> Identity | None:
    """Change a password inside the TUI; forced mode is used on first login."""
    mode = "forced-password" if forced else "password"
    values = {"current": "", "password": "", "confirm": ""}
    message = ""

    while True:
        submitted = _run_form(
            mode,
            values,
            database=_database_name(db_path),
            message=message,
        )
        if submitted is None:
            return None if forced else identity

        if not forced and authenticate(db_path, identity.username, submitted["current"]) is None:
            values = dict(submitted)
            values["current"] = ""
            message = "当前密码错误。"
            continue

        password = submitted["password"]
        if password != submitted["confirm"]:
            values = dict(submitted)
            values["password"] = ""
            values["confirm"] = ""
            message = "两次密码不一致。"
            continue

        try:
            updated = set_password(db_path, identity, password)
        except ValueError as error:
            values = dict(submitted)
            values["password"] = ""
            values["confirm"] = ""
            message = str(error)
            continue

        write_session(updated, db_path)
        if not forced:
            _wait_message(
                mode,
                submitted,
                "密码已修改。",
                database=_database_name(db_path),
            )
        return updated


def frame(
    mode: str,
    values: Mapping[str, str],
    *,
    active: str = "",
    cursor: int | None = None,
    message: str = "",
    database: str = "xingyuan.db",
    phase: float = 0.35,
) -> screen.ScreenFrame:
    """Render one authentication frame from state only."""
    terminal = screen._terminal_size()
    width, height = max(1, terminal.columns - 1), max(5, terminal.lines)
    fields = _fields(mode)
    left, top, field_width = _layout(width, height, len(fields))

    board = Board(width, height)
    board.put(0, 0, _topbar(width, database))
    board.put(0, height - 1, theme.footer(width))

    board.put(
        left,
        top,
        _title(mode),
        screen._BOLD + screen._TEXT_ACCENT,
        width=field_width,
    )

    field_top = top + 2
    label_width = min(_LABEL_WIDTH, max(1, field_width // 3))
    box_width = max(4, field_width - label_width - 2)
    protected_cells: set[tuple[int, int]] = set()

    for index, spec in enumerate(fields):
        row = field_top + index * 2
        if row >= height - 1:
            break

        value = str(values.get(spec.key, ""))
        label_style = screen._TEXT_PRIMARY if spec.key == active else screen._TEXT_SECONDARY
        label_text = screen._ansi(screen._pad_cells(spec.label, label_width), label_style)

        if spec.editable:
            inner_width = max(1, box_width - 2)
            buffer = TextBuffer.from_value(value)
            if spec.key == active and cursor is not None:
                buffer.cursor = max(0, min(cursor, len(buffer.chars)))
            shown, _ = buffer.view(inner_width, secret=spec.secret)
            box_text = screen._pad_cells(" " + shown, box_width)
            box_style = (
                screen._SURFACE_SELECTED + screen._TEXT_ON_SELECTED
                if spec.key == active
                else screen._SURFACE_INTERACTIVE + screen._TEXT_PRIMARY
            )
            line = label_text + "  " + screen._ansi(box_text, box_style)
            box_x = left + label_width + 2
            protected_cells.update((row, box_x + offset) for offset in range(box_width))
        else:
            value_text = screen._ansi(screen._clip_cells(value, box_width), screen._TEXT_PRIMARY)
            line = label_text + "  " + value_text

        board.put(left, row, line, width=field_width)

    message_row = field_top + len(fields) * 2
    if message and message_row < height - 1:
        board.put(left, message_row, message, screen._TEXT_ACCENT, width=field_width)

    return animation._starlight(board.frame(), width, phase, protected_cells)


def _initialize_admin(db_path: Path | str | None) -> bool:
    values = {"username": ADMIN_USERNAME, "password": "", "confirm": ""}
    message = ""
    while True:
        submitted = _run_form(
            "initialize",
            values,
            database=_database_name(db_path),
            message=message,
        )
        if submitted is None:
            return False
        if submitted["password"] != submitted["confirm"]:
            values = {"username": ADMIN_USERNAME, "password": "", "confirm": ""}
            message = "两次密码不一致。"
            continue
        try:
            initialize_admin(submitted["password"])
        except ValueError as error:
            values = {"username": ADMIN_USERNAME, "password": "", "confirm": ""}
            message = str(error)
            continue
        clear_session()
        return True


def _run_form(
    mode: str,
    initial_values: Mapping[str, str],
    *,
    database: str,
    message: str = "",
) -> dict[str, str] | None:
    """Run one animated form with a single state/render/event loop."""
    fields = _fields(mode)
    editable = [spec for spec in fields if spec.editable]
    if not editable:
        return dict(initial_values)

    values = {spec.key: str(initial_values.get(spec.key, "")) for spec in fields}
    buffers = {
        spec.key: TextBuffer.from_value(values[spec.key])
        for spec in editable
    }
    active_index = 0
    previous_lines: list[str] = []

    try:
        with input_mode():
            while True:
                active = editable[active_index]
                buffer = buffers[active.key]
                for spec in editable:
                    values[spec.key] = buffers[spec.key].value

                rendered = frame(
                    mode,
                    values,
                    active=active.key,
                    cursor=buffer.cursor,
                    message=message,
                    database=database,
                    phase=_phase(),
                )
                _paint_form(
                    rendered.lines,
                    previous_lines,
                    _cursor_position(mode, active, buffer),
                )
                previous_lines = rendered.lines

                event = read_event(animation._SPARKLE_FRAME)
                if event is None:
                    continue

                if event.kind == "cancel":
                    return None
                if event.kind in {"up", "down", "tab"}:
                    step = -1 if event.kind == "up" else 1
                    active_index = (active_index + step) % len(editable)
                    continue
                if event.kind == "submit":
                    if active_index < len(editable) - 1:
                        active_index += 1
                        continue
                    for spec in editable:
                        values[spec.key] = buffers[spec.key].value
                    return values

                buffer.apply(event)
    except (KeyboardInterrupt, EOFError):
        return None
    finally:
        _hide_cursor()


def _cursor_position(
    mode: str,
    active: FieldSpec,
    buffer: TextBuffer,
) -> tuple[int, int]:
    terminal = screen._terminal_size()
    width, height = max(1, terminal.columns - 1), max(5, terminal.lines)
    fields = _fields(mode)
    left, top, field_width = _layout(width, height, len(fields))
    field_index = next(index for index, spec in enumerate(fields) if spec.key == active.key)
    row = min(height - 2, top + 2 + field_index * 2)
    label_width = min(_LABEL_WIDTH, max(1, field_width // 3))
    box_width = max(4, field_width - label_width - 2)
    inner_width = max(1, box_width - 2)
    _, cursor_cells = buffer.view(inner_width, secret=active.secret)
    box_x = left + label_width + 2
    return box_x + 1 + cursor_cells, row


def _paint_form(
    lines: list[str],
    previous: list[str],
    cursor: tuple[int, int],
) -> None:
    tty = sys.stdout.isatty()
    if tty:
        sys.stdout.write("\x1b[?25l")
        sys.stdout.flush()
    screen._paint(lines, previous)
    if tty:
        x, y = cursor
        sys.stdout.write(f"\x1b[{y + 1};{x + 1}H\x1b[?25h")
        sys.stdout.flush()


def _hide_cursor() -> None:
    if sys.stdout.isatty():
        sys.stdout.write("\x1b[?25l")
        sys.stdout.flush()


def _wait_message(
    mode: str,
    values: Mapping[str, str],
    message: str,
    *,
    database: str,
) -> None:
    previous_lines: list[str] = []
    while True:
        rendered = frame(
            mode,
            values,
            message=message,
            database=database,
            phase=_phase(),
        )
        screen._paint(rendered.lines, previous_lines)
        previous_lines = rendered.lines
        try:
            key = keys._read_key(animation._SPARKLE_FRAME)
        except (KeyboardInterrupt, EOFError):
            return
        if key in {"select", "back"}:
            return


def _phase() -> float:
    return time.monotonic() * 0.85


def _fields(mode: str) -> tuple[FieldSpec, ...]:
    if mode == "initialize":
        return (
            FieldSpec("username", "账号", editable=False),
            FieldSpec("password", "设置密码", secret=True),
            FieldSpec("confirm", "确认密码", secret=True),
        )
    if mode == "login":
        return (
            FieldSpec("username", "账号"),
            FieldSpec("password", "密码", secret=True),
        )
    if mode == "forced-password":
        return (
            FieldSpec("password", "设置密码", secret=True),
            FieldSpec("confirm", "确认密码", secret=True),
        )
    return (
        FieldSpec("current", "当前密码", secret=True),
        FieldSpec("password", "设置密码", secret=True),
        FieldSpec("confirm", "确认密码", secret=True),
    )


def _title(mode: str) -> str:
    if mode == "initialize":
        return "管理员设置"
    if mode == "login":
        return "登录"
    if mode == "forced-password":
        return "设置新密码"
    return "修改密码"


def _layout(width: int, height: int, field_count: int) -> tuple[int, int, int]:
    field_width = min(_FIELD_WIDTH, max(12, width - 4))
    left = max(0, (width - field_width) // 2)
    body_height = 3 + field_count * 2
    top = max(1, (height - 1 - body_height) // 2)
    return left, top, field_width


def _topbar(width: int, database: str) -> str:
    return theme.topbar(width, database=database)


def _database_name(db_path: Path | str | None) -> str:
    return Path(db_path).name if db_path else "xingyuan.db"
