...
class UnifiedContractTests(unittest.TestCase):
    def setUp(self):
        auth = patch("xingyuan_sis.auth_cli.require_identity", return_value=Identity("Administrator", "admin"))
        auth.start()
    ...
    def test_csv_and_form_share_partial_birth_date_validation(self):
        ...
        path.write_text(
            ...
            f"00991001,坏日期,{row['family']},{row['branch']},2026,2007-02-30,18\n"
            f"00991002,只有生日,{row['family']},{row['branch']},2026,--02-29,18\n"
            f"00991003,完整生日,{row['family']},{row['branch']},2026,2008-02-29,99\n",
            encoding='utf-8',
        )
        ...
        self.assertEqual(self.service.student_by_no('00991002')['birth_date'], '--02-29')
        self.assertEqual(self.service.student_by_no('00991002')['age'], 18)
        self.assertEqual(self.service.student_by_no('00991003')['birth_date'], '2008-02-29')
        self.assertIsNone(self.service.student_by_no('00991003')['age'])
