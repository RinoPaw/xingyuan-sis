...
class InlineEditIsolationTests(unittest.TestCase):
    ...
    def test_student_edit_only_projects_the_selected_field_group(self):
        ...
        baseline = self._student_text(student_inspector._lines(row, self.catalog))

        workspace._open_field(state, self.catalog, "enrollment_year")
        editing = self._student_text(student_inspector._lines(row, self.catalog, state))
    ...
    def test_generic_inspectors_do_not_reformat_unrelated_fields_during_edit(self):
        ...
        for collection, field_key in cases:
            with self.subTest(collection=collection):
                ...
                row = state.current(self.catalog)
                baseline = self._generic_text(detail.details(collection, row, self.catalog, 42))

                workspace._open_field(state, self.catalog, field_key)
                editing = self._generic_text(detail.details(collection, row, self.catalog, 42, state))
