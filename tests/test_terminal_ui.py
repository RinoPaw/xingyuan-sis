...
from xingyuan_sis import basic_ui, menu, terminal_ui
...
class KeyboardMenuTests(unittest.TestCase):
    def test_plain_keys_and_interrupts(self) -> None:
        for char, expected in (("\r", "select"), (" ", "select"), ("j", "down"),
            ...
            with self.subTest(char=char):
                self.assertEqual(menu._plain_key(char), expected)
        with self.assertRaises(KeyboardInterrupt):
            menu._plain_key("\x03")
        with self.assertRaises(EOFError):
            menu._plain_key("")

    def test_navigation_wraps_and_supports_shortcuts(self) -> None:
        cases = [(["up", "select"], 2), (["end", "home", "select"], 0),
                 (["9", "2"], 1), (["back"], None)]
        for keys, expected in cases:
            with self.subTest(keys=keys), patch.object(menu, "_clear"), \
                 patch.object(menu, "_read_key", side_effect=keys), redirect_stdout(StringIO()):
                self.assertEqual(menu._select("测试", ["一", "二", "三"]), expected)

    def test_submenu_remembers_selection_and_survives_cancelled_action(self) -> None:
        action = Mock(side_effect=KeyboardInterrupt)
        with patch.object(menu, "_select", side_effect=[1, None]) as select, \
             patch.object(menu, "_clear"), redirect_stdout(StringIO()):
            menu._menu("学生", [("列表", Mock()), ("编辑", action)])
        self.assertEqual(select.call_args_list[1].kwargs["selected"], 1)
    ...
    def test_home_remembers_selection_after_return(self) -> None:
        with patch.object(menu.sys.stdin, "isatty", return_value=True), \
             ...
             patch.object(menu, "_courses") as courses, patch.object(menu, "_clear"), \
        ...
        for size in ((80, 24), (40, 16), (30, 12), (120, 30)):
            for selected in range(6):
                with self.subTest(size=size, selected=selected), \
                     patch.object(menu, "_terminal_size", return_value=os.terminal_size(size)):
                    lines = menu._home_lines(LABELS, selected, {}, 1.0)
                self.assertEqual(len(lines), size[1])
                self.assertTrue(all(menu._display_width(line) < size[0] for line in lines))
                for label in LABELS:
                    self.assertIn(label, "\n".join(lines))
    ...
    def test_animation_and_single_bottom_bar_at_every_size(self) -> None:
        for size in ((30, 12), (40, 16), (80, 20), (80, 24), (160, 48)):
            with self.subTest(size=size), patch.object(menu, "_terminal_size", return_value=os.terminal_size(size)):
                ...
                self.assertNotIn("\n", first[-1])
                self.assertLess(menu._display_width(first[-1]), size[0])
    ...
    def test_pausing_animation_and_interrupt_restore_cursor(self) -> None:
        ...
        with redirect_stdout(output), patch("sys.stdout.isatty", return_value=True), \
             ...
             patch.object(menu, "_read_key", side_effect=["pause", KeyboardInterrupt]), \
    ...
    def test_key_typed_before_read_is_preserved(self) -> None:
        ...
        try:
            ...
            with patch("sys.stdin.fileno", return_value=slave):
                self.assertEqual(menu._read_key_posix(timeout=0.5), "2")
    ...
    def test_escape_sequences_and_terminal_restore(self) -> None:
        ...
        for sequence, expected in ((b"[A", "up"), (b"OB", "down"), (b"[H", "home"),
            ...
            with self.subTest(sequence=sequence), patch("sys.stdin.fileno", return_value=10), \
                 ...
                 patch.object(menu, "_read_escape_sequence", return_value=sequence):
                self.assertEqual(menu._read_key_posix(), expected)
                import termios
...
class InteractionTests(unittest.TestCase):
    ...
    def test_long_chinese_lines_wrap_without_losing_content(self) -> None:
        ...
        self.assertTrue(all(menu._display_width(line) <= 10 for line in lines))
