...
宽屏采用“名册 + 档案”双栏；窄屏仍使用同一状态机，只改变排布。键盘焦点与窄屏当前内容面板是两个独立状态，因此焦点进入操作栏不会让正在查看的名册或档案消失。

常用操作：

- `Tab`：单向循环切换学生列表 / 档案 / 操作栏
- `↑ / ↓`：沿当前区域的空间关系移动
...
`Shift+Tab` 不承担反向切换；工作台只保留一个 Tab 焦点循环。
...
当前 seed 包含 4 个学院、10 个专业、26 个班级、113 名学生、20 门课程和 452 条选课 / 成绩记录。
