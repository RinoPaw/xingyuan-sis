...
学生档案包含学号、姓名、族系、支系、性别、年龄、出生资料、入学年份、班级、学籍状态、主元素亲和与等级等信息。出生资料支持完整日期、仅年份或仅月日；完整日期存在时年龄实时计算，否则可以保存独立年龄。学院和专业通过班级关系获得，不在学生表重复保存。
...
`Esc` 统一返回或取消当前层。已有记录直接在档案中选择字段并按 Enter 原地修改；文本输入或选项再次按 Enter 后立即校验并保存，Esc 取消当前字段。字段编辑没有独立保存按钮；关联字段显示名称与业务编号，可从列表选择。新建记录仍使用完整表单，删除和演示数据操作需要确认。沿关联记录浏览后返回，会恢复原记录、筛选、滚动位置和档案焦点，即使记录编号已修改。

表单、CLI 和 CSV 共用必填项、出生资料与数值校验：出生资料接受有效的 `YYYY-MM-DD`、`YYYY` 或 `--MM-DD`，入学年份为 1900～9999，年龄为非负整数，成绩为 0～100 或空值；`NaN`、无穷值和非整数课时会明确报错。CSV 保留逐行错误报告，合法行可以正常导入。
...
- **就地纠错**：编辑年份、年龄、学分、课时、成绩时，输入无效数字会提示重填；留空保持原值，成绩可输入 `-` 清空。
...
xy stu edit 20260001 --age 24 --birth=--05-13
xy stu rm 20260001
...
当前 seed 包含 4 个学院、10 个专业、26 个班级、114 名学生、20 门课程和 456 条选课/成绩记录。学生角色只要求来源明确；年龄与出生资料仅作为角色资料，不限制入学，具体来源和记录规则见 [演示数据来源](docs/demo-data-sources.md)。
...
│   ├── data-model.md            # 数据结构与字段语义
│   ├── demo-data-sources.md     # 演示角色、年龄与物种来源
│   ├── design-principles.md     # 当前架构设计原则
│   ├── record-presentation.md   # 档案展示 / 编辑的统一值模型
│   └── ui-design.md             # TUI 视觉与交互契约
├── tests/
├── CONTRIBUTING.md
├── pyproject.toml
└── README.md
