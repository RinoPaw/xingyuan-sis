...
↑↓/滚轮 移动  Enter/点击 打开  p 暂停  q/0 退出
```

点击首页模块后直接进入数据工作台：

- **学生**：左侧名册，右侧即时档案和关联选课。点击记录或用方向键移动便会更新预览，无需先选择查询功能或输入学号。
- **课程**：课程目录与选课名单并排显示，关联成绩可以直接打开。
- **成绩**：切换全部、待录入、已评分视图；编辑时自动绑定当前学生、课程和学期。
- **教务**：通过页内标签切换学院、专业、班级，沿关联记录浏览校园组织。
- **数据**：学生元素分布、班级人数与成绩进度直接呈现，提供 CSV 导入、导出和演示校园入口。

宽屏采用名册与档案双栏；窄屏点击记录或按 Enter / Tab 展开档案，`q` 返回名册。顶部路径中的上级页面可以点击，点击“首页”直接返回首页。`1` / `2` / `3` 切换页内视图，`/` 打开搜索，支持中文及多个关键词；`a` 新建、`e` 编辑、`d` 删除、`r` 刷新。方向键浏览记录，滚轮滚动鼠标所在的名册或档案区域；Tab 切换键盘焦点，Home / End 跳到首项 / 末项。

编辑器保留当前名册，在侧栏中暂存更改。点击字段或按 Enter 输入；班级、学院、学生、课程等关联字段可直接按名称选择。`s` 保存，`q` 取消，取消不会写入数据库。删除和演示数据操作会先显示确认页面。短窗口会滚动字段或切换到紧凑视图。数据页使用 `i` 导入、`o` 导出、`g` 建立演示校园；演示数据只会写入空数据库。

首页按 `p` 暂停或播放动画；本次运行会记住设置。窗口缩放直接读取 PTY 的实际尺寸，背景重绘和鼠标点击区域随布局更新。即时键盘菜单在独立终端屏幕中运行，并临时统一终端底色；退出后恢复原来的屏幕与终端配置配色。边缘配色需要终端支持 OSC 11 / 111，设置 `NO_COLOR` 时不修改配色。

代码结构和扩展边界见 [项目结构](docs/architecture.md)。

基础菜单和共享输出辅助仍支持以下操作：

- **点击与滚轮**：默认键盘菜单支持点击首页导航、路径中的上级页面、名册行和底栏按钮；滚轮可切换菜单选项、滚动查询结果。需要终端发送 SGR 鼠标事件；不支持时仍可使用数字键和方向键。进入表单后关闭鼠标捕获，保留终端原生中文输入与粘贴。
- **输入框**：默认键盘菜单的表单使用深灰输入行和浅色文字，继续采用系统逐行输入；基础菜单和直接 CLI 保留朴素输入。
