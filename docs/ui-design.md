...
一级菜单获得焦点时，只有当前一级项表达焦点，例如 `▌ 教务`。右侧二级菜单此时只是**预览**：所有二级按钮保持普通 `surface-interactive + text-primary`，即使程序内部记住了上一次二级选择，也不得显示 `·`、蓝色文字或其他“当前项”样式，以免制造第二个焦点。

进入二级菜单后，一级项降为弱上下文 `▏ 教务`；真正获得键盘焦点的二级项使用 `surface-selected + text-on-selected`，不再额外添加 `>` / `›` 等箭头。也就是说：`▌` 表示当前一级焦点，`▏` 表示当前所属一级栏目；二级焦点由蓝色文字与选中表面共同表达。

无论控件使用导航竖条、整行背景或输入框背景来辅助表达，**真正的当前焦点 / 选中文字统一使用蓝色强调色**。不得再出现一级菜单蓝色、二级按钮白色、工作区选中行白色等页面级配色分叉；这些状态统一通过 `text-accent` / `text-on-selected` 映射到同一个颜色。
...
登录、首页与工作台统一使用 `theme.topbar`：左侧为 `✦ 星原 SIS`，右侧身份和数据库信息属于次级文字。字段标签定宽对齐，关联字段显示名称和业务编号。一级导航用竖条表达层级焦点；按钮、名册行、详情操作和选择器使用蓝色文字配合选中表面，不再额外堆叠箭头或第二根竖条。
...
状态强调不应只靠单一前景色：一级导航还使用层级竖条，列表和按钮使用选中表面，文本输入使用输入框表面。避免为同一个状态重复叠加多个相同作用的符号。
