...
进入二级菜单后，一级项降为弱上下文 `▏ 教务`；真正获得键盘焦点的二级项使用 `surface-selected + text-on-selected` 和 `›` 标记。也就是说：`▌` 表示当前一级焦点，`▏` 表示当前所属一级栏目，`›` 表示当前二级焦点。

无论控件使用 `▌`、`›`、整行灰底或输入框背景来辅助表达，**真正的当前焦点 / 选中文字统一使用蓝色强调色**。不得再出现一级菜单蓝色、二级按钮白色、工作区选中行白色等页面级配色分叉；这些状态统一通过 `text-accent` / `text-on-selected` 映射到同一个颜色。
