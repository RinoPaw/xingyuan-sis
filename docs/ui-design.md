...
林岚  20260001 ...               ← 当前记录使用 surface-selected
  ...
  档案 / 即时预览                ← 未获得焦点，仍保持正常标题层级
林岚                             ← 当前记录标题 text-accent + bold
20260001                         ← identifier text-secondary
```

宽屏双栏必须明确区分“记录选择”和“面板焦点”。左侧高亮行表示当前记录，即使焦点切到右侧也继续保留；`Tab` 只切换哪一侧接收键盘操作。获得焦点的面板标题使用 `▌` + `text-accent` + 粗体，另一侧标题保持 `text-primary`。
...
右侧所有能够执行动作的项目（关联记录、可编辑字段、成绩等）属于同一个键盘选择序列。获得右侧焦点后，`↑/↓` 在这些可交互项之间移动，当前项使用与名册选中行一致的 `surface-selected + text-on-selected`，`Enter` 执行当前项。滚动时应保持当前选择与可见区域一致，不能只改变文字状态却不给出明确的选中反馈。
